...
def learning_module(pipe, ):
    ...
    saving_iterator = 0
    general_iterator = 0
    interval_saving_q = 300 # =>original value 500
...
def save_q(Q, iteration=-1):
    if iteration > 0:
        file_path = '/home/pfe-vm/Pfe-SDN/logs/{}/Q_array.json'.format(iteration)
    else:
        file_path = '/home/pfe-vm/Pfe-SDN/logs/Q_array.json'
    with open(file_path, 'w') as file:
        json.dump(Q, file)  # use `json.loads` to do the reverse


def save_q_best(Q):
    with open('/home/pfe-vm/Pfe-SDN/Q_array_best.json', 'a') as file:
        json.dump(Q, file)  # use `json.loads` to do the reverse
